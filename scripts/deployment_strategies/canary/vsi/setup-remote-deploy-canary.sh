...
Instace_Group_CRN_ID1=$(ibmcloud is instance-groups -json | jq -r ".[] | select(.name==\"$Instace_Group_NAME1\") | .crn")
Instace_Group_CRN_ID2=$(ibmcloud is instance-groups -json | jq -r ".[] | select(.name==\"$Instace_Group_NAME2\") | .crn")
...
Install_App_IG() {
  ...
  for i in "${Member_Id[@]}"; do
    ...
    curl -sSL "https://raw.githubusercontent.com/anamikaagrawal123/commons/master/scripts/deployment_strategies/basic/vsi/backup.sh" --output nackup.sh
    ...
    curl -sSL "https://raw.githubusercontent.com/anamikaagrawal123/commons/master/scripts/deployment_strategies/basic/vsi/cleanup.sh" --output cleanup.sh
